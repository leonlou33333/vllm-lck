...
class DeepseekV2ForCausalLM(
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        layer_name_pattern = re.compile(r"^model\.layers\.(\d+)\.")
        max_supported_layers = self.config.num_hidden_layers + max(
            0, getattr(self.config, "num_nextn_predict_layers", 0)
        )

        def should_skip_pruned_layer(weight_name: str) -> bool:
            match = layer_name_pattern.match(weight_name)
            if match is None:
                return False
            return int(match.group(1)) >= max_supported_layers
        ...
        for name, loaded_weight in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                if should_skip_pruned_layer(name) or name not in params_dict:
                    continue
                ...
                for j in range(num_chunks):
                    ...
                    for mapping in expert_params_mapping:
                        ...
                        if (
                            should_skip_pruned_layer(name_mapped)
